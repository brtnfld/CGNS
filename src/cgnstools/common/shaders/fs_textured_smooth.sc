/*
 * fs_textured_smooth.sc
 * Textured smooth shading fragment shader for CGNS visualization
 * Combines texture mapping with per-vertex Blinn-Phong lighting
 */
...
void main()
{
	...
	if (u_enableLighting.x > 0.5) {
		...
		// Combine lighting with color (from texture or vertex)
